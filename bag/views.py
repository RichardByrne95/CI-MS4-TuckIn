...
def add_to_bag(request):
    try:
        ...
        restaurant = get_object_or_404(
            Restaurant, menusection__name=menu_section.name)
        ...
        if bag_keys:
                ...
                if food_id in list(bag[restaurant.name].keys()):
                    ...
                    if additional_details:
                        ...
                        if bag[restaurant.name][food_id]['additional_details']:
                            bag[restaurant.name][food_id]['additional_details'] += ', ' + \
                                additional_details
                        # Else add additional details
                        else:
                            bag[restaurant.name][food_id]['additional_details'] += additional_details
                    messages.success(
                        request, f'Updated quantity of {food.friendly_name} to {bag[restaurant.name][food_id]["quantity"]}')
                # Else add food and set quantity
                else:
                    bag[restaurant.name][food_id] = {
                        'quantity': quantity, 'additional_details': additional_details}
                    messages.success(
                        request, f'Added {bag[restaurant.name][food_id]["quantity"]} {food.friendly_name} to your cart')
            # Else if there is food from another restaurant in the bag, throw error (elif used for extra verification)
                ...
                messages.error(
                    request, 'There is already food from another restaurant in your cart.')
        # Else add food to bag
        else:
            bag[restaurant.name] = {
                food_id: {'quantity': quantity, 'additional_details': additional_details}}
            # If quantity being added is more than 1, say quantity in toast
            if quantity > 1:
                messages.success(
                    request, f'Added {quantity} of {food.friendly_name} to your cart')
            else:
                messages.success(
                    request, f'Added {food.friendly_name} to your cart')
...
def remove_from_bag(request, food_id):
    try:
        ...
        messages.info(
            request, f'{food.friendly_name} has been removed from your order')
...
def order_again(request, order_number):
    try:
        ...
        for item in order.lineitems.all():
            ...
            menu_section = get_object_or_404(
                MenuSection, fooditem__name=food_object.name)
            restaurant = get_object_or_404(
                Restaurant, menusection__name=menu_section.name)
                    ...
                    if food_id in list(bag[restaurant_name].keys()):
                        ...
                        if additional_details:
                            ...
                            if bag[restaurant_name][food_id]['additional_details']:
                                bag[restaurant_name][food_id]['additional_details'] += ', ' + \
                                    additional_details
                            # Else add additional details
                            else:
                                bag[restaurant_name][food_id]['additional_details'] += additional_details
                        messages.success(
                            request, f'Updated quantity of {food_object.friendly_name} to {bag[restaurant_name][food_id]["quantity"]}')
                    # Else add food and set quantity
                    else:
                        bag[restaurant_name][food_id] = {
                            'quantity': quantity, 'additional_details': additional_details}
                        messages.success(
                            request, f'Added {bag[restaurant_name][food_id]["quantity"]} {food_object.friendly_name} to your cart')
                # Else if there is food from another restaurant in the bag, throw error (elif used for extra verification)
            ...
            # Else add food to bag
                ...
                if quantity > 1:
                    messages.success(
                        request, f'Added {quantity} of {food_object.friendly_name} to your cart')
                else:
                    messages.success(
